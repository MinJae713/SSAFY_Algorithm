 ...
 * 	1.2. 반복시 마다 visited 초기화
 * 	1.3. number 번째 사람 visited true 지정
 * 2. number 위치 사람을 시작으로 5명까지 들어갈 수 있는지 확인(getRelation)
 * 	2.1. 파라미터는 탐색 사람 번호(number), 들어온 깊이(depth)
 * 	2.2. 들어온 깊이가 5라면 true 반환
 * 	2.3. 해당 번호와 인접한 번호(relationPerson)들에 대해 반복
 * 		2.3.1. 이미 방문한 번호라면 다음 반복 실행
 * 		2.3.2. 방문하지 않았다면 relationPerson 방문 여부 true 지정
 * 		2.3.3. 해당 번호와 depth+1를 파라미터로 재귀 호출
 * 		2.3.4. 호출 이후, relationPerson 방문 여부 false 지정
 * 			2.4.3.1. 다음 반복 시 다른 위치에서 들어갈 수 있게 하기 위함임
 * 		2.3.5. 재귀 호출 결과가 true라면 true 반환
 * 	2.5. false 반환
 * 3. number 번째 사람 visited false 지정
 */
public class ABCDE {
	static class Person {
		int number;
...
//		System.setIn(new FileInputStream("sample_input.txt"));
	...
	private static boolean getRelation(int number, int depth) {
		...
		for (Person relationPerson=relations[number]; 
			...
			visited[relationPerson.number] = true;
			boolean hasFound = getRelation(relationPerson.number, depth+1);
